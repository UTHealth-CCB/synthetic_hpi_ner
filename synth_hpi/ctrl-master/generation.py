...
parser.add_argument('--print_once', action='store_true', default=1,
...
parser.add_argument('--sample_num', type=int, default=1,
                                        help='generated number of samples with generated_num length')
parser.add_argument('--output_txt', type=str, default='output_ctrl.txt', help='output ctrl text')
...
print('sample_num: {} generate_num: {}'.format(sample_num, generate_num))
generated_samples = 0
output_pfn = args.output_txt
out_f = open(output_pfn, 'w', encoding='utf-8')
while True:
    generated_samples += 1
    if generated_samples > sample_num:
        print('generated {} samples.'.format(sample_num))
        break
    ...
    try:
        for token in range(len(text)-1, args.generate_num-1):
          ...
          if args.topn > 0 :
            print('TOPN :: chosen word:', idx2word[idx], end=" ")
          ...
          words_generated = [idx2word[c] for c in tokens_generated[0].squeeze()[:token+2]]
          tokens_generated_so_far = ' '.join(words_generated)
          tokens_generated_so_far = re.sub('(@@ )', '', string=tokens_generated_so_far)
          tokens_generated_so_far = re.sub('(@@ ?$)', '', string=tokens_generated_so_far)              

          #if not args.print_once:
          #  print('---------------------------------------')
          #  print(tokens_generated_so_far, end=" ")          
          #  print()
          print(words_generated[-2], end=" ")
        out_f.write(tokens_generated_so_far)
        print('{}/{}: ---------------------------------------'.format(generated_samples, sample_num))            
